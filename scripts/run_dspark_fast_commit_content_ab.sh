...
TOKENS=${TOKENS:-64}
OUT_ROOT=${OUT_ROOT:-./quality-results/dspark-fast-commit-content-ab}
...
mkdir -p "$OUT_DIR"
TMP_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/ds4-fast-content.XXXXXX")
...
echo "FAST_COMMIT_CONTENT_SETUP source_run=$SOURCE_RUN_DIR confidence=$CONFIDENCE tokens=$TOKENS"
...
echo "FAST_COMMIT_CONTENT_SOURCE_CHECK status=PASS"

MAKE_JOBS=${MAKE_JOBS:-4}
make -C "$TRACE_WT" -j "$MAKE_JOBS" ds4 >/dev/null
TRACE_BIN="$TRACE_WT/ds4"
[[ -x "$TRACE_BIN" ]] || { echo "content trace build did not produce $TRACE_BIN" >&2; exit 2; }
echo "FAST_COMMIT_CONTENT_BUILD_CHECK status=PASS"
...
run_arm() {
  ...
  echo "FAST_COMMIT_CONTENT_RUN arm=$label fast_full_commit=$fast"
  (
...
python3 "$CONTENT_TOOL" analyze \
  --fast-log "$OUT_DIR/E4_FAST.log" \
  --replay-log "$OUT_DIR/E4_REPLAY.log" | tee "$OUT_DIR/content-ab-summary.log"

echo "FAST_COMMIT_CONTENT_RESULTS_DIR=$OUT_DIR"
