...
OUT_ROOT=${OUT_ROOT:-./quality-results/dspark-fast-commit-draft-verify-input-ab}
...
for f in "$MODEL" "$DSPARK_MODEL" "$PROMPT_FILE" "$STATE_TOOL" "$E10_TOOL"; do
...
mkdir -p "$OUT_DIR"
TMP_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/ds4-fast-draft-verify-input.XXXXXX")
...
echo "FAST_COMMIT_DRAFT_VERIFY_SETUP source_run=$SOURCE_RUN_DIR confidence=$CONFIDENCE tokens=$TOKENS expected_ordinal1=$EXPECTED_FIRST_DIFF_ORDINAL"
git -C "$ROOT_DIR" worktree add --detach "$TRACE_WT" HEAD >/dev/null
WORKTREE_ADDED=1

python3 "$STATE_TOOL" instrument --source "$TRACE_WT/ds4.c"
python3 "$E10_TOOL" instrument --source "$TRACE_WT/ds4.c"

grep -q 'DS4_DSPARK_STATE_RETURN' "$TRACE_WT/ds4.c" || { echo "E10 state-return instrumentation missing" >&2; exit 2; }
grep -q 'ds4_e10_verify_suffix_tops' "$TRACE_WT/ds4.c" || { echo "E10 verifier wrapper missing" >&2; exit 2; }
grep -q 'DS4_DSPARK_E10_VERIFY' "$TRACE_WT/ds4.c" || { echo "E10 verify sentinel missing" >&2; exit 2; }
echo "FAST_COMMIT_DRAFT_VERIFY_SOURCE_CHECK status=PASS"

MAKE_JOBS=${MAKE_JOBS:-4}
make -C "$TRACE_WT" -j "$MAKE_JOBS" ds4 >/dev/null
TRACE_BIN="$TRACE_WT/ds4"
[[ -x "$TRACE_BIN" ]] || { echo "E10 build did not produce $TRACE_BIN" >&2; exit 2; }
echo "FAST_COMMIT_DRAFT_VERIFY_BUILD_CHECK status=PASS"

PROMPT=$(cat "$PROMPT_FILE")

echo "FAST_COMMIT_DRAFT_VERIFY_RUN arm=SEQUENTIAL_ORACLE"
(
  ...
  unset DS4_DSPARK_E10_TRACE DS4_DSPARK_TRACE_STATE_AB
...
run_arm() {
  ...
  echo "FAST_COMMIT_DRAFT_VERIFY_RUN arm=$label fast_full_commit=$fast trace=$trace"
  (
    unset DS4_FAMILY_REPAIRS
    unset DS4_FAMILY1_REPAIR DS4_FAMILY2_REPAIR DS4_FAMILY3_REPAIR
    unset DS4_FAMILY4_REPAIR DS4_FAMILY5_REPAIR DS4_FAMILY6_REPAIR DS4_FAMILY7_REPAIR
    ...
    unset DS4_METAL_Q8_DECODE_MPP
    if [[ "$trace" == 1 ]]; then
      export DS4_DSPARK_E10_TRACE=1
      export DS4_DSPARK_TRACE_STATE_AB=1
    fi
...
run_arm E10_REPLAY_TRACE 0 1
run_arm E10_FAST_BASE 1 0
run_arm E10_FAST_TRACE 1 1

python3 "$E10_TOOL" analyze \
  --ds4 "$TRACE_BIN" \
  --model "$MODEL" \
  --oracle "$OUT_DIR/SEQUENTIAL_ORACLE.txt" \
  --replay-out "$OUT_DIR/E10_REPLAY_TRACE.txt" \
  --replay-log "$OUT_DIR/E10_REPLAY_TRACE.log" \
  --fast-base-out "$OUT_DIR/E10_FAST_BASE.txt" \
  --fast-base-log "$OUT_DIR/E10_FAST_BASE.log" \
  --fast-trace-out "$OUT_DIR/E10_FAST_TRACE.txt" \
  --fast-trace-log "$OUT_DIR/E10_FAST_TRACE.log" \
  --expected-ordinal1 "$EXPECTED_FIRST_DIFF_ORDINAL" \
  | tee "$OUT_DIR/draft-verify-input-ab-summary.log"

echo "FAST_COMMIT_DRAFT_VERIFY_RESULTS_DIR=$OUT_DIR"
